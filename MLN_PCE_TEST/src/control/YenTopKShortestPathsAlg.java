...
public class YenTopKShortestPathsAlg
{
	...
	public Path next()
	{
		...
		for(int i=0; i<count-1; ++i)
		{
			...
			BaseVertex cur_succ_vertex = cur_result_path.get_vertices().get(cur_dev_vertex_id+1);
			
//			_graph.remove_edge(new Pair<Integer,Integer>(cur_derivation.get_id(), cur_succ_vertex.get_id()));
			_graph.remove_edge(new Pair<Long, Long>(cur_derivation.get_id(), cur_succ_vertex.get_id()));
		}
...
//			_graph.remove_edge(new Pair<Integer, Integer>(cur_path_vertex_list.get(i).get_id(), cur_path_vertex_list.get(i+1).get_id()));
			_graph.remove_edge(new Pair<Long, Long>(cur_path_vertex_list.get(i).get_id(), cur_path_vertex_list.get(i+1).get_id()));
		}
...
//			_graph.recover_removed_edge(new Pair<Integer, Integer>(cur_recover_vertex.get_id(), succ_vertex.get_id()));
			_graph.recover_removed_edge(new Pair<Long, Long>(cur_recover_vertex.get_id(), succ_vertex.get_id()));
